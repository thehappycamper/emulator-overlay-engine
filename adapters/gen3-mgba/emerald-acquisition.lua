-- Read-only acquisition proof for Pokemon Emerald (English retail Rev 0).
...
local output = console:createBuffer("Emerald acquisition")
...
local function updateDiagnostic()
    ...
    local text = string.format(
        '{"diagnosticVersion":"0.1.0","status":"ok","game":{"gameCode":%s,"title":%s,"revision":%d,"crc32":"%08X"},"party":{"count":%d,"first":%s},"battle":{"active":%s,"typeFlags":%d,"opponent":%s},"location":%s}',
        jsonString(identity.gameCode),
        jsonString(identity.title),
        identity.revision,
        identity.crc32,
