...
3. While working, keep Implementation Notes current enough that another agent could resume the task cold. Isolate concurrent work via a separate worktree or clone (see `AGENTS.md`'s Concurrent Agents rule) and record that in `Worktree/Clone Isolation`. Each active task branch has one implementation owner; reviewers inspect it from isolated checkouts and do not modify it.
