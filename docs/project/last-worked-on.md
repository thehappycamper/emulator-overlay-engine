...
- **Active task:** [`P05-T006` — Automate Emerald Proof 1 savestate loading via mGBA's CLI](../tasks/P05/P05-T006.md), implemented on `feat/P05-T006-emerald-savestate-cli-launch` and awaiting independent review.
...
- **Next P05 step:** review `P05-T006`, then run and record the first real mGBA end-to-end acceptance test using `npm run proof:emerald` (config validation, mGBA launch, and now savestate loading are automated; the mapper and dev-server terminals still require the printed copy-paste commands; script loading remains the one confirmed-unavoidable manual step). Broader phase-gate acquisition/mapping (whole party, moves/bag, seed/frame, species/map lookup data) remains after that smoke. `P05` remains active and is not complete until that real live test is performed and recorded.

## Most recent session (2026-08-08)

`P05-T006` investigated mGBA's actual documented capabilities rather than continuing to assume none existed: confirmed via the mGBA maintainers' own "closed as not planned" resolution of [mgba-emu/mgba#3289](https://github.com/mgba-emu/mgba/issues/3289) that Lua-script auto-loading has no supported non-GUI path, but found `mgba-qt` officially documents a `--savestate` command-line flag that `P05-T005` wasn't using. Savestate loading is now automated via that flag; script loading remains the one genuinely unavoidable manual step, now explained with authoritative evidence instead of an unexplained caveat.
