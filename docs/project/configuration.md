...
## Emerald Memory Explorer (developer tool, P05-T008)

`npm run explore:launch -- --provider bizhawk|mgba [--check] [--scan-start 0x02000000 --scan-length 256]` reuses the same `.env.bizhawk.local`/`.env.local` executable/ROM/savestate paths, but launches with a separate, read-only diagnostic connector (`adapters/{bizhawk,gen3-mgba}/memory-explorer-connector.lua`) instead of the production one. It generates a watch list from `EMERALD_US_REV0.addresses` (not a second hand-maintained list) and writes it to `var/snapshots/emerald-memory-explorer.watches.lua`. For mGBA, load the printed connector path manually — mGBA still has no supported way to auto-load a script.

In a second terminal, `npm run explore:emerald` starts a local, read-only server (default `http://127.0.0.1:5190`) showing three panels: raw watched memory (hex + typed interpretations + change highlighting since the last poll), the provider-neutral Emerald source snapshot (if `npm run proof:emerald`/`proof:bizhawk` is also running), and the normalized `public/live-state.json` (if `npm run live:emerald` is also running). This tool never writes to emulator memory or to any file the production path reads — it only proxies already-written local JSON files over GET. Candidate field labels typed into the UI are stored in the browser only (`localStorage`), never sent anywhere.

This is explicitly developer-only tooling for verifying/discovering acquisition fields, not part of the production overlay pipeline.
