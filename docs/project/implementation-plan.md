...
**Status:** active. `P05-T001` through `P05-T007`, and `P05-T009` through `P05-T013` are complete. The launcher now automates supported savestate loading while Lua script loading remains manual. `P05-T012` replaced the three-terminal BizHawk proof workflow with a single `npm run proof:emerald:bizhawk` session command on a reusable, emulator-agnostic orchestration primitive. `P05-T013` establishes the isolated child-process Libretro provider prototype. `P05-T014` connects that provider to the existing Emerald acquisition pipeline via a thin adapter, proving a third provider-neutral acquisition path end to end (real-core smoke reaches boot state only, since neither the provider nor this adapter can load a savestate); implemented, awaiting independent review. Neither makes Libretro the production-default provider. The phase gate still requires real mGBA (and, per `P06`, BizHawk) end-to-end acceptance evidence against this richer pipeline with actual in-game save data. `P05` remains active and is not complete until the real live acceptance test is performed and recorded.
...
- [`P05-T014`](../tasks/P05/P05-T014.md) - Libretro Emerald thin adapter: connects `P05-T013`'s isolated provider to the existing, unmodified Emerald acquisition/mapping pipeline via a synchronous reader backed by bounded IPC snapshot reads, file-hash ROM identity, and fail-closed core/region checks. Three-way mGBA/BizHawk/Libretro provider parity proven. Implemented, pushed to `feat/P05-T014-libretro-emerald-adapter`, awaiting independent review. See [ADR 0028](../decisions/0028-libretro-emerald-thin-adapter.md).
