...
**Status:** active. `P05-T001` through `P05-T006`, `P05-T009`, `P05-T010`, and `P05-T011` are complete. The launcher now automates supported savestate loading while Lua script loading remains manual. `P05-T007` is an independent, non-gating feasibility spike exploring a Libretro direct-host acquisition path alongside (not instead of) the mGBA/BizHawk work, and is awaiting independent re-review. The phase gate still requires real mGBA (and, per `P06`, BizHawk) end-to-end acceptance evidence against this richer pipeline. `P05` remains active and is not complete until the real live acceptance test is performed and recorded.
...
- [`P05-T007`](../tasks/P05/P05-T007.md) - Libretro direct-host feasibility spike: dynamically loads the real mGBA libretro core via Node/FFI (no RetroArch, Lua, GUI automation, or process hooking) and verifies the ABI plumbing up to `retro_load_game`. Implemented, two independent-review fix rounds applied; awaiting independent re-review after a documentation-accuracy fix. Explicitly non-gating and independent of the mGBA/BizHawk acquisition work — a parallel research track, not a replacement.
