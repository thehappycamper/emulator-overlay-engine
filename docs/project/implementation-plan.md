...
| `P04` | Event Routing And Actions | active - `P04-T001` and `P04-T002` completed; `P04-T003` is the next narrow local integration task |
...
**Exit criteria:** A minimal, provable path from a detected event to a validated declarative rule result and action request exists (met by `P04-T001`). A capability-gated executor turns at least one such request into a real, harmless local effect, failing closed on every unsupported/unauthorized/invalid case (met by `P04-T002`). Broad action-provider ecosystem integrations remain out of scope for this phase.
