...
| `P05` | Live Gameplay Source | active - `P05-T001`-`T007`, `T009`-`T013`, `T015`-`T016` completed; `T017` in review; real mGBA/BizHawk end-to-end acceptance still required |
...
**Status:** active. `P04-T001` proves declarative event-to-action-request evaluation, and `P04-T002` adds the capability-gated action executor and two harmless local action providers; both are completed after independent review and merge. `P04-T003` wires `overlay.notification` to a real local delivery channel (pokemon.fainted -> rule -> executor -> browser notification); completed after independent review and merge. `P04-T004` restores the real browser app shell (dashboard + notifications load through native ESM with no Node-only dependency); completed after independent review and merge. Broad action-provider ecosystem integrations remain out of scope.
...
- [`P04-T004`](../tasks/P04/P04-T004.md) — Restore browser app-shell module resolution by keeping Node-only event validation out of the browser domain descriptor. Completed (independent review APPROVE, merged at `819f4b2`). See [ADR 0030](../decisions/0030-browser-domain-module-boundary.md).
...
**Status:** active. `P05-T001` through `P05-T007`, and `P05-T009` through `P05-T013`, `P05-T015`, and `P05-T016` are complete; `P05-T017` is in review. The phase gate still requires real mGBA/BizHawk end-to-end acceptance evidence against the richer pipeline. `P05` remains active and is not complete until the real live acceptance test is performed and recorded.
...
- [`P05-T017`](../tasks/P05/P05-T017.md) — Preserve acquired Emerald stat stages through normalized state and expose Gen III stage-adjusted ordinary battle stats in Compare Stats. In review; real BizHawk acceptance remains outstanding.
