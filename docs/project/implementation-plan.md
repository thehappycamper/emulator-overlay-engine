...
- [`P05-T016`](../tasks/P05/P05-T016.md) - Provider-neutral `player.party.defeated` semantic event (wired through the existing P04 rule/action/executor path to an `overlay.notification`) plus live Emerald battle stat-stage acquisition/presentation (`gBattleMons`, resolving the fixed-address gap `P05-T010` explicitly left open, via an independently cross-checked community linker/RAM map). Awaiting independent review; does not touch `activePlayerIndex`/active-battler-slot identity, schema bounds, or the proof-session orchestrator.
